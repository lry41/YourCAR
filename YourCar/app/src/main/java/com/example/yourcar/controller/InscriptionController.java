    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        inscription.setOnClickListener(new View.OnClickListener() {
            ...
            //database = FirebaseDatabase.getInstance();
                ...
                if (nomText.isEmpty()){
                    erreurInscr.setText("veuillez rentrez votre prénom");
                    nom.requestFocus();
                    return;
                }
                if (prenomText.isEmpty()){
                    erreurInscr.setText("veuillez rentrez votre nom");
                    prenom.requestFocus();
                    return;
                }
                if (emailText.isEmpty()){
                    erreurInscr.setText("veuillez rentrez votre email");
                    email.requestFocus();
                    return;
                }
                if (!Patterns.EMAIL_ADDRESS.matcher(emailText).matches()){
                    erreurInscr.setText("votre email n'est pas conforme");
                    email.requestFocus();
                    return;
                }
                if (numTelText.isEmpty()){
                    erreurInscr.setText("veuillez rentrez votre numéro");
                    numeroTel.requestFocus();
                    return;
                }
                if (passwordText.isEmpty()){
                    erreurInscr.setText("veuillez rentrez votre mot de passe");
                    password.requestFocus();
                    return;
                }
                if (passwordText.length()<6){
                    erreurInscr.setText("6 charactères minimum pour le mot de passe");
                    password.requestFocus();
                    return;
                }
